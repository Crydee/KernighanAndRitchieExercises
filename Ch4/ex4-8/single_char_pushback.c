...
/* Modified getch and ungetch: Since we know that there will only ever be one character pushed back,
 * we can replace the buffer that was used to store the pushed back characters with a single character
 * variable.  Since we know that we won't push back the EOF character, we can use it to tell if there
 * is a pushed back character that we should return or not.*/

int pushed_back = EOF; /* buffer for ungetch */

int getch(void) /* get a (possibly pushed back) character */
{
  if (pushed_back != EOF) { /* Check if the value of pushed_back has been set, and return it's value.*/
   int return_value = pushed_back; 
   pushed_back = EOF;
   return return_value;
  }
  return getchar();
...
{
  if (pushed_back != EOF) { /* We're assuming that only one character will be pushed back at a time, but check anyway.*/
    printf("Error, trying to push back more than one character.\n");
    return;
  }
  pushed_back = c;
}
