...
/* The only change in this exercise is to change the type of the buffer from a buffer of characters to a buffer of integers.
 * The reason for this is that the EOF indicator may be a negative integer, and the C specification doesn't specify that
 * characters be signed or unsigned, so how the translation from a negative int, to a character, and then back to an int
 * to be compared to EOF is not well defined.  We can avoid this by storing the pushed back characters in a datatype 
 * that is strictly larger than the char data type and that can handle negative values.*/
